...
if ($currentGuildId === '') {
    fwrite(STDERR, "DISCORD_GUILD_ID must be configured for automatic sync." . PHP_EOL);
    exit(1);
}
...
try {
    $stmt = $pdo->prepare('SELECT *
        ...
        LIMIT 1');
    $stmt->execute(['guild_id' => $currentGuildId]);
    $row = $stmt->fetch();

    if (!$row) {
        fwrite(STDOUT, "This configured guild is not currently eligible for automatic sync." . PHP_EOL);
        exit(0);
    }

    $guildId = trim((string)($row['discord_guild_id'] ?? $currentGuildId));
    if ($guildId === '') {
        $guildId = $currentGuildId;
    }

    fwrite(STDOUT, sprintf('[%s] Running auto sync for guild %s%s', gmdate('Y-m-d H:i:s'), $guildId, PHP_EOL));
    ...
    try {
        ...
        fwrite(STDERR, 'Auto sync failed: ' . $e->getMessage() . PHP_EOL);
        exit(1);
    }
} finally {
